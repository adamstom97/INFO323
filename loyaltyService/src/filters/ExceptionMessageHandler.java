...
public class ExceptionMessageHandler implements ExceptionMapper<Exception> {
	...
	public Response toResponse(Exception ex) {
		...
		// exception messages are not included in responses by default, so this
		// code explicitly extracts them, and inserts them into the response.
		if (ex instanceof WebApplicationException) {
			WebApplicationException wex = (WebApplicationException) ex;
			return Response.status(wex.getResponse().getStatus()).entity(
					  wex.getMessage()).type("text/plain").build();
		} else {
